...
const siswaTsv = fs.readFileSync('data-siswa.tsv', 'utf-8');
const guruTsv = fs.readFileSync('data-guru.tsv', 'utf-8');
...
fs.writeFileSync('musada-sd-default-rtdb-export.json', JSON.stringify(rtdb, null, 2));
console.log('Successfully generated musada-sd-default-rtdb-export.json, Teachers:', Object.values(rtdb.users).filter(u => u.role !== 'ortu').length, 'Students:', Object.values(rtdb.users).filter(u => u.role === 'ortu').length);
